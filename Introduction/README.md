...
prompt> ./cpu A
```
Using the bash shell, run this program in multiple processes like this:
```
prompt> ./cpu A & ./cpu B & ./cpu C & ./cpu D &
```
To terminate each of these background processes, bring each to the foreground
one-by-one using this command:
```
prompt> fg
```
and then press ctrl-c to terminate the foreground process.
